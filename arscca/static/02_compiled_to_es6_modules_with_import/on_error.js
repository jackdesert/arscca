let on_error = () => {
    ...
    var sendDataToEngineers = function (details) {
        'use strict';
        const url = '/javascript_errors', data_json = JSON.stringify(details), request = new XMLHttpRequest();
        request.open('POST', url, true);
        request.setRequestHeader('Content-Type', 'application/json');
        request.onload = function () {
            if (request.status >= 200 && request.status < 400) {
                // Success!
                var respJson = request.responseText, resp = JSON.parse(respJson), alertUser = resp.alert_user;
